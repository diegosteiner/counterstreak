...
export function goalProgress(count: number, goal: number): number {
  ...
  if (goal < 0) {
    const start = -goal;
    fraction = start === 0 ? 1 : (start - count) / start;
  } else {
    fraction = goal === 0 ? 1 : count / goal;
  }
...
export function formatDuration(ms: number): string {
  ...
  if (totalSec < 60) return `${totalSec}${t("duration.second")}`;
...
function isPeriod(v: unknown): v is Period {
  return typeof v === "string" && (PERIODS as unknown as string[]).includes(v);
}
...
class CounterStore {
  ...
  update(
    ...
    if (changes.name !== undefined)
      item.name = changes.name.trim() || "Counter";
    ...
    if (
      changes.period !== undefined &&
      isPeriod(changes.period) &&
      changes.period !== item.period
    ) {
