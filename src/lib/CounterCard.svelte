...
>
    <div class="flex w-full items-center justify-between gap-2">
        <div class="flex-1 text-left text-[1.1rem] font-semibold">
            <span>{item.name}</span>
            <a
                class="cursor-pointer text-muted no-underline hover:text-text"
    ...
    <div class="flex w-full items-center gap-4">
        ...
        <div class="flex min-w-0 flex-1 items-baseline justify-center gap-[0.4rem]">
            <output class="text-[clamp(2.25rem,12vw,3.5rem)] font-extrabold leading-none tabular-nums"
                >{item.count}</output
            >
            <span class="text-[0.95rem] tabular-nums {reached ? 'text-success' : 'text-dim'}"
