...
<main class="flex min-h-dvh flex-col items-center gap-2 p-6 text-center">
	<header class="flex w-full max-w-[30rem] items-center justify-center gap-4">
	</header>
	...
	<ul class="flex w-full max-w-[30rem] list-none flex-col gap-5">
		...
		{/each}
		<li
				class="flex flex-col items-center gap-2 rounded-2xl border-2 p-5 transition-colors border-transparent bg-surface">
				<a class="hover:underline" href="{base}/edit/new"
					>{t('home.addOne')}</a
				></li>
	</ul>
	<footer class="mt-auto flex items-center gap-4 pt-4">
		<select
